...
- **Reset** — `reset_current_command()` recomputes default/unset values for the current command's flags (honoring spec-declared string flag defaults), clears its argument values, and propagates global flag resets across command levels via `sync_global_flag()`.
